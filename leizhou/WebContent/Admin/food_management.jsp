	...
	function allDelete() {
		...
		if (flag) {		
			if (confirm("你确定要删除这些记录吗?"))
				location.href = "foodManageServlet?action=deleteCheckFood&arr=" + arr; //调用servlet			
		} else
...
<body>
	...
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="role_management_add('添加食物','food_management_add.jsp','900','500')">
		...
		<!-- <form class="layui-form x-right " action="foodManageServlet?action=findFoodLike" method="post"
			style="width: 500px; margin-top:-2px; height: 1px">	 -->	
			<div style="width:520px;height:30px;float:right;display:inline-block">
				<form class="layui-form x-right " action="foodManageServlet?action=findFoodLike" method="post" style="float:left;display:inline-block">
					<select name="ftid" style="height:28px;float:left;font-size:17px;border:1px solid #e6e6e6;display:block;">
						<option value="" selected></option>
						<option value="1" >美味早餐</option>
						<option value="2" >风味小吃</option>
						<option value="4" >特色菜</option>
					</select>
					<div class="layui-input-inline" style="width: 300px;float:left">
						<input type="text" name="fname" placeholder="搜索内容"
		...
		<tbody>
			<c:if test="${!empty foodList }">
				<c:forEach items="${foodList}" var="foodList" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="check" value=${foodList.fid }></td>
						<c:if test="${1==foodList.ftid }">
							<td>美味早餐</td>
						</c:if>
						<c:if test="${2==foodList.ftid }">
							<td>风味小吃</td>
						</c:if>
						<c:if test="${4==foodList.ftid }">
							<td>特色菜</td>
						</c:if>
						<td>${foodList.fname }</td>
						<td><img style="width:50%;height:50px;margin-top: 10px;margin-left: 68px;" 
							src="${pageContext.request.contextPath }/${foodList.image }" /></td>
						<%-- <td>${foodList.aid }</td> --%>
						<td class="td-manage"><a title="修改" href="javascript:;"
							onclick="role_management_update('修改食物','foodManageServlet?action=findUpdateFood&fid=${foodList.fid }','900','500')"
							class="ml-5" style="text-decoration: none"> <i
								class="layui-icon"><img src="images/look2.png" width="15"
									height="15"></i>
						</a> &ensp;
								<a href ="foodManageServlet?action=deleteFood&fid=${foodList.fid }"> <img src="images/shanchu.png" width="15"
									height="15">
						</a></td>
					</tr>
				</c:forEach>
