...
<body>
	...
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="role_management_add('添加公告','notice_management_add.jsp','900','500')">
		...
		<form class="layui-form x-right " action="admNoticeServlet?action=findNoticeByname"
		...
		<tbody>
			<c:if test="${!empty noticeList }">
				<c:forEach items="${noticeList}" var="noticeList" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="check" value=${noticeList.nid }></td>
						<td>${noticeList.nid }</td>
						<td>${noticeList.ntid}</td>
						<td>${noticeList.nname }</td>
						<td>${noticeList.aid }</td>

           
						<td class="td-manage"><a title="修改" href="javascript:;"
							onclick="role_management_update('修改公告','admNoticeServlet?action=findupdateNotice&nid=${noticeList.nid }','900','500')"
							...
							class="ml-5" style="text-decoration: none"> <i
								class="layui-icon"> --%>
								<a href ="admNoticeServlet?action=deleteNotice&nid=${noticeList.nid }"> <img src="images/shanchu.png" width="15"
									height="15">
						</a></td>
					</tr>
				</c:forEach>
