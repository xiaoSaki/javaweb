	...
	function allDelete() {
		...
		if (flag) {		
			if (confirm("你确定要删除这些记录吗?"))
				location.href = "userManageServlet?action=deleteCheckUser&arr=" + arr; //调用servlet			
		} else
...
<body>
	...
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="role_management_add('添加用户','user_management_add.jsp','900','500')">
		...
		<!-- <button class="layui-btn layui-btn-danger">
			<i class="layui-icon"><img src="images/daochu.png" width="15"
				height="15"></img></i>批量导出
		</button>
		<button class="layui-btn layui-btn-danger">
		...
		<form class="layui-form x-right " action="userManageServlet?action=findUserByName" method="post"
			style="width: 500px; margin-top:-2px; height: 2px">
			<div class="layui-input-inline" style="width: 400px">
				<input type="text" name="name" placeholder="搜索内容"
		...
		<tbody>
			<c:if test="${!empty userList }">
				<c:forEach items="${userList}" var="userList" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="check" value=${userList.id }></td>
						<td>${userList.id }</td>
						<td>${userList.name }</td>
						<td>${userList.passwd }</td>
						<td>${userList.age }</td>
						<td>${userList.sex }</td>
						<td>${userList.email }</td>
						<c:if test="${1==userList.status }">
							<td>管理员</td>
						</c:if>
						<c:if test="${0==userList.status }">
							<td>普通用户</td>
						</c:if>

           
						<td class="td-manage"><a title="修改" href="javascript:;"
							onclick="role_management_update('修改用户','userManageServlet?action=findupdateUser&id=${userList.id }','900','500')"
							class="ml-5" style="text-decoration: none"> <i
								class="layui-icon"><img src="images/look2.png" width="15"
									height="15"></i>
						</a> &ensp;<%-- <a title="删除" href="javascript:;"
							onclick="role_management_delete('删除','userManageServlet?action=deleteUser&id=${userList.id }','0','','0')"
							class="ml-5" style="text-decoration: none"> <i
								class="layui-icon"> --%>
								<a href ="userManageServlet?action=deleteUser&id=${userList.id }"> <img src="images/shanchu.png" width="15"
									height="15">
						</a></td>
					</tr>
				</c:forEach>
