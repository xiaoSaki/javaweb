...
<body>
	...
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="role_management_add('添加文章类型','article_management_add.jsp','900','500')">
		...
		<form class="layui-form x-right " action="<%=basePath %>/Admin/admArticleServlet?action=findArticleByid"
			style="width: 500px; height: 2px">
			<div class="layui-input-inline" style="width: 400px">
				<input type="text" name="aid" placeholder="搜索内容"
					autocomplete="off" class="layui-input" style="height: 30px">
			</div>
			<div class="layui-input-inline" style="width: 80px">
				<!-- <submit class="layui-btn" lay-submit="" lay-filter="sreach">
	...
	<table class="layui-table">
		...
		<tbody>
			<c:if test="${!empty articleList }">
				<c:forEach items="${articleList}" var="articleList" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="check" value=${articleList.aid }></td>
						<td>${articleList.aid  }</td>
						<td>${articleList.atid}</td>
						<td>${articleList.articlename }</td>
						<td>${articleList.publisher }</td>
						<td>${articleList.releasetime}</td>
						<td>${articleList.content }</td>
						<td>${articleList.image}</td>
           
						<td class="td-manage"><a title="修改" href="javascript:;"
							onclick="role_management_update('修改文章','admArticleServlet?action=findupdateArticle&aid=${articleList.aid }','900','500')"
							class="ml-5" style="text-decoration: none"> <i
								class="layui-icon"><img src="images/look2.png" width="15"
									height="15"></i>
						</a> &ensp;<%-- <a title="删除" href="javascript:;"
							onclick="role_management_delete('删除','articleManageServlet?action=deleteUser&id=${articleList.id }','0','','0')"
							class="ml-5" style="text-decoration: none"> <i
								class="layui-icon"> --%>
								<a href ="admArticleServlet?action=deleteArticle&aid=${articleList.aid }"> <img src="images/shanchu.png" width="15"
									height="15">
						</a></td>
					</tr>
				</c:forEach>
