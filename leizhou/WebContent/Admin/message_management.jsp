	...
	function allDelete() {
		...
		if (flag) {		
			if (confirm("你确定要删除这些记录吗?"))
				location.href = "messageManageServlet?action=deleteCheckMessage&arr=" + arr; //调用servlet			
		} else
...
<body>
	...
	<div class="x-body">
		...
		<button class="layui-btn"
			onclick="role_management_add('添加留言','message_management_add.jsp','900','500')">
		...
		<form class="layui-form x-right " action="messageManageServlet?action=findMessageByName" method="post"
		...
		<tbody>
			<c:if test="${!empty messageList }">
				<c:forEach items="${messageList}" var="messageList" varStatus="loop">
					<tr>
						<td><input type="checkbox" name="check" value=${messageList.mid }></td>
						<td>${messageList.name }</td>
						<td>${messageList.sex }</td>
						<td>${messageList.tel }</td>
						<td>${messageList.message }</td>					         
						<td class="td-manage"><a title="修改" href="javascript:;"
							onclick="role_management_update('修改留言','messageManageServlet?action=findupdateMessage&mid=${messageList.mid }','900','500')"
							class="ml-5" style="text-decoration: none"> <i
								class="layui-icon"><img src="images/look2.png" width="15"
									height="15"></i>
						</a> &ensp;
							<a href ="messageManageServlet?action=deleteMessage&mid=${messageList.mid }"> <img src="images/shanchu.png" width="15"
								height="15">
						</a></td>
					</tr>
				</c:forEach>
